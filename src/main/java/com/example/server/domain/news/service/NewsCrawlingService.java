...
public class NewsCrawlingService {
    ...
    private final ArticleTitleResolver articleTitleResolver;

    private final NewsCrawlingProperties props;

    @Value("${news.crawling.per-category-size:5}")
    ...
    public int crawlCategory(NewsCategory category) {
        ...
        for (NaverNewsItem item : res.items()) {
            ...
            if (articleTitleResolver.looksTruncated(title)) {
                String full = articleTitleResolver.resolveFullTitleOrFallback(sourceLink, title);
                if (full != null && !full.isBlank()) title = full;
            }
